...
static char* SAVEFILE_NAME = "tetris_quicksave.bin";
...
static int TryQuickload(TetrisDisplay *windows, TetrisGameState *s) {
  ...
  if (!SanityCheckState(&tmp)) {
    StatusPrintf(windows, "Invalid %s contents", SAVEFILE_NAME);
    return 0;
  }
...
int main(int argc, char **argv) {
	TetrisDisplay disp;
  	...
  	SetupCurses();
 	CreateWindows(&disp);
  	DisplayGameState(&disp, &s);
  	RefreshAllWindows(&disp);
  	printf("\x1b[>");
  	return 0;
}
